...
def process_two_plus(args, left_rules, right_rules):
    result = rds.walk_rule_diff_stats(args, left_rules, right_rules, rds.two_plus_oval, rds.two_plus_remediation)
...
def process_prodtypes(args, left_rules, right_rules):
    result = rds.walk_rule_diff_stats(args, left_rules, right_rules, rds.prodtypes_oval, rds.prodtypes_remediation)
    affected_rules = result[0]
    affected_ovals = result[1]
    affected_remediations = result[2]
    affected_remediations_type = result[4]
...
def process_product_names(args, left_rules, right_rules):
    result = rds.walk_rule_diff_stats(args, left_rules, right_rules, rds.product_names_oval, rds.product_names_remediation)
    affected_rules = result[0]
    affected_ovals = result[1]
    affected_remediations = result[2]
    affected_remediations_type = result[4]
...
def process_introspection(args, left_rules, right_rules):
    for rule_id in args.query:
        if not args.summary_only:
            pprint.pprint(left_rules, right_rules[rule_id])
            print("\n")
        else:
            print(rule_id)


def process_unassociated(args, left_rules, right_rules, all_products):
    ...
    for rule_id in left_rules, right_rules:
        rule_obj = left_rules, right_rules[rule_id]
...
def main():
    ...
    if args.introspect and args.query:
        process_introspection(args, left_rules, right_rules)
    if args.unassociated:
        process_unassociated(args, left_rules, right_rules, all_products)
